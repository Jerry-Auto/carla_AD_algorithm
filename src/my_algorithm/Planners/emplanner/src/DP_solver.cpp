...
PathCostFunction::PathCostFunction(const WeightCoefficients& weights,
                        const PathPlannerConfig& config,
                                 const std::vector<general::SLObstacle>& obstacles)
    : _config(config), _weights(weights), _static_obstacles(obstacles) {}

double PathCostFunction::calculateTransitionCost(
    SLState& from, 
    SLState& to
)  {
    // local cost accumulator (not used directly here)
    ...
    for (int i = 1; i < static_cast<int>(linspace.size()); ++i) {
        double s = linspace[i];
...
double PathCostFunction::calculateObstacleCost(const std::vector<double> s_set, const std::vector<double> l_set) const {
    ...
    for (size_t i = 0; i < s_set.size(); ++i) {
        double s_sample = s_set[i];
        double l_sample = l_set[i];
        // 计算最小距离
        double min_distance = std::numeric_limits<double>::max();
        for (const auto& obs : _static_obstacles) {
            double distance = obs.minDistanceTo(s_sample, l_sample);
            if (distance < min_distance) {
                min_distance = distance;
            }
        }
        // 根据距离计算代价
        if (min_distance < _weights.safety_margin) {  
            // 在安全边界内，高代价
            cost += _weights.path_dp_w_obs*1000.0 / (min_distance*min_distance + 1e-6); // 避免除零
        }
        // 在安全边界外，代价为0
    }
    return cost;
...
bool PathConstraintChecker::isCollidingWithObstacle(const SLState& state, 
                                                  const general::SLObstacle& obs) const {
    // 直接使用SLObstacle的contains方法进行碰撞检测
    return obs.contains(state.s, state.l, obs.safety_margin);
}
...
std::vector<std::vector<SLState>> PathSamplingStrategy::generateSamplingGrid(
    ...
    (void)num_layers; // Suppress unused parameter warning
...
SpeedCostFunction::SpeedCostFunction(const WeightCoefficients& weights,
                                   ...
                                   const std::vector<general::STObstacle>& obstacles) 
...
double SpeedCostFunction::calculateTransitionCost(
    ...
    // 参考速度代价（每点）
    cost_ref_speed += _weights.speed_dp_w_ref_speed * std::pow(s_dot - _reference_speed, 2)*_dp_poly_pnt_num;
    // 加速度硬约束 + 二次代价
    if (s_dot_dot > 4.0 || s_dot_dot < -6.0) {
        // 超出物理极限：施加极大惩罚
        cost_a += 1e6;
    } else {
        cost_a += _weights.speed_dp_w_a * std::pow(s_dot_dot, 2)*_dp_poly_pnt_num;
    }
    // Jerk 代价
    cost_jerk = _weights.speed_dp_w_jerk * std::pow(s_dot_dot_dot, 2)*_dp_poly_pnt_num;

    // 2. 障碍物代价：基于线性插值采样
    std::vector<std::pair<double, double>> sample_points;
    for (int i = 0; i < static_cast<int>(t_linspace.size()); ++i) {
...
double SpeedCostFunction::calculateObstacleCost(
    ...
    for (const auto& sample : sample_points) {
        ...
        if (min_distance <= _weights.safety_margin) {
            // 平方反比关系
            cost_obs += _weights.speed_dp_w_obs*1000.0 / (min_distance * min_distance+ 1e-6); // 避免除零
        }
        // 距离大于安全距离，代价为0
    }
    return cost_obs;
...
bool SpeedConstraintChecker::checkState(const STState& state) const {
    ...
    // 4. 检查是否与障碍物冲突
    for (const auto& obs : _st_obstacles) {
        if (obs.contains(state.t, state.s,obs.safety_margin)) {
            return false;
        }
    }
    return true;
...
bool SpeedConstraintChecker::checkInterpolatedPath(const STState& from, const STState& to) const {
    ...
    for (int i = 1; i <= num_intervals; ++i) {
        ...
        // 检查每个插值点
        for (const auto& obs : _st_obstacles) {
            if (obs.contains(t, s,obs.safety_margin)) {
...
std::vector<STState> SpeedSamplingStrategy::generateNextLayer(
    ...
    (void)current_layer; // Suppress unused parameter warning
    std::vector<STState> next_states;
    for(size_t i=0;i<_s_serier.size();++i){
        STState next_state;
...
std::vector<double> SpeedSamplingStrategy::_sample_s_t_method(double max_border,double init_step) const {
    ...
    int count = 0;
    while (current <= max_border && serier.size() < static_cast<size_t>(max_points)) { // Cast max_points to size_t
