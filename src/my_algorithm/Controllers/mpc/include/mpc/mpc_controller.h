...
#include "pidlqr/pid_controller.h"
...
private:
    ...
    void compute_target_point(const std::vector<general::TrajectoryPoint>& trajectory,
                              ...
                              general::TrajectoryPoint& target_point, double& target_point_s);

    // 更新状态向量
    void update_state(const std::vector<general::TrajectoryPoint>& trajectory,
                      const std::shared_ptr<general::VehicleState>& ego_state,
                      const double predicted_time);
