...
Obstacle::Obstacle()
    : id(0), x(0.0), y(0.0), z(0.0), heading(0.0),
      vx(0.0), vy(0.0), vz(0.0),
      length(5.0), width(2.0), height(1.5),
      time_stamp(0.0),
      polygon(nullptr) {}

std::shared_ptr<Polygon2d> Obstacle::BuildPolyNow() const {
  return CollisionDetection::get_bounding_box(*this);
}

bool Obstacle::contains(double px, double py, double safety_margin) const {
  Vec2d point(px, py);
  auto poly_ptr = BuildPolyNow();
  return CollisionDetection::distance_to(poly_ptr, point) <= safety_margin;
}

double Obstacle::minDistanceTo(double px, double py) const {
  Vec2d point(px, py);
  auto poly_ptr = BuildPolyNow();
