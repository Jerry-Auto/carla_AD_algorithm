...
cilqr::cilqr(std::shared_ptr<cost_base> cost_func) : cost_func_(cost_func) {
    std::cout << "cilqr constructor start" << std::endl;
    // logger_ = std::make_shared<AD_algorithm::general::Logger>("CILQR");
    // logger_->info("CILQR solver initializing...");
    std::cout << "cost_func_ is null: " << (cost_func_ == nullptr) << std::endl;
    // init();
    // build_function(); // Temporarily commented out for debugging
    std::cout << "cilqr constructor end" << std::endl;
}
...
void cilqr::init(){
    ...
    Qf_ = Eigen::MatrixXd::Zero(nx_, nx_);
    terminal_ref_ = Eigen::VectorXd::Zero(nx_);
    ref_ = Eigen::MatrixXd::Zero(N_ + 1, nx_);
    obstacles_vector_ = Eigen::VectorXd::Zero(50);
    num_obstacles_ = 0;
}

void cilqr::build_function() {
    // Combined [x; u] form
    ...
    // Parameters
    casadi::SX Q = casadi::SX::sym("Q", nx_, nx_);
    casadi::SX R = casadi::SX::sym("R", nu_, nu_);
    casadi::SX ref = casadi::SX::sym("ref", nx_, 1);
    casadi::SX Qf = casadi::SX::sym("Qf", nx_, nx_);
    casadi::SX terminal_ref = casadi::SX::sym("terminal_ref", nx_, 1);
    casadi::SX obstacles_vector = casadi::SX::sym("obstacles_vector", 50, 1); // max 10 obstacles * 5
    casadi::SX num_obstacles = casadi::SX::sym("num_obstacles", 1);

    // Dynamics
    casadi::SX next_state = cost_func_->dynamics(state, control);
    f_ = casadi::Function("f", std::vector<casadi::SX>{z}, std::vector<casadi::SX>{next_state});
    
    casadi::SX f_jac_mx = casadi::SX::jacobian(next_state, z);
    f_jacobian_ = casadi::Function("f_jac", std::vector<casadi::SX>{z}, std::vector<casadi::SX>{f_jac_mx});
    ...
    casadi::SX f_hess_mx = casadi::SX::hessian(f_weighted, z);
    f_hessian_ = casadi::Function("f_hess", std::vector<casadi::SX>{z, v_x}, std::vector<casadi::SX>{f_hess_mx});
    
    // Cost functions
    casadi::SX l_stage = cost_func_->cost_function(state, control, Q, R, ref);
    l_ = casadi::Function("l", std::vector<casadi::SX>{z, Q, R, ref}, std::vector<casadi::SX>{l_stage});
    
    casadi::SX l_jac_mx = casadi::SX::jacobian(l_stage, z);
    l_jacobian_ = casadi::Function("l_jac", std::vector<casadi::SX>{z, Q, R, ref}, std::vector<casadi::SX>{l_jac_mx});
    
    casadi::SX l_hess_mx = casadi::SX::hessian(l_stage, z);
    l_hessian_ = casadi::Function("l_hess", std::vector<casadi::SX>{z, Q, R, ref}, std::vector<casadi::SX>{l_hess_mx});


    // Terminal cost
    casadi::SX term_state = casadi::SX::sym("term_state", nx_);
    casadi::SX l_term = cost_func_->terminal_cost(term_state, Qf, terminal_ref);
    terminal_l_ = casadi::Function("terminal_l", std::vector<casadi::SX>{term_state, Qf, terminal_ref}, std::vector<casadi::SX>{l_term});
    
    casadi::SX term_jac_mx = casadi::SX::jacobian(l_term, term_state);
    terminal_l_x_ = casadi::Function("term_l_x", std::vector<casadi::SX>{term_state, Qf, terminal_ref}, std::vector<casadi::SX>{term_jac_mx});
    
    casadi::SX term_hess_mx = casadi::SX::hessian(l_term, term_state);
    terminal_l_xx_ = casadi::Function("term_l_xx", std::vector<casadi::SX>{term_state, Qf, terminal_ref}, std::vector<casadi::SX>{term_hess_mx});

    // Constraints (for ALM). Infer constraint count
    casadi::SX c_vec = cost_func_->constraints(state, control, obstacles_vector, num_obstacles);
    c_ = casadi::Function("c", std::vector<casadi::SX>{z, obstacles_vector, num_obstacles}, std::vector<casadi::SX>{c_vec});
    
    // try {
    //     casadi::DM z0_dm = casadi::DM::zeros(nx_ + nu_, 1);
    //     casadi::DM obstacles_dm = casadi::DM::zeros(50,1);
    //     casadi::DM num_obs_dm = casadi::DM::zeros(1,1);
    //     auto c0 = c_(std::vector<casadi::DM>{z0_dm, obstacles_dm, num_obs_dm});
    //     if (c0.size() > 0) {
    //         casadi::DM c0dm = c0[0];
    //         if (c0dm.size2() == 1) {
    ...
    //     num_c_ = 0;
    // }
    num_c_ = 28; // Fixed number of constraints
    ...
    if (num_c_ > 0) {
        ...
        cost_vec_barrier_ = casadi::Function(
            "cost_vec_barrier", std::vector<casadi::SX>{z, rho_b, obstacles_vector, num_obstacles},
        ...
        cost_vec_alm_ = casadi::Function(
            "cost_vec_alm", std::vector<casadi::SX>{z, mu_vec, rho_alm, obstacles_vector, num_obstacles},
        ...
        cost_sum_barrier_ = casadi::Function(
            "cost_sum_barrier", std::vector<casadi::SX>{z, rho_b, obstacles_vector, num_obstacles},
        ...
        cost_sum_barrier_jac_ = casadi::Function(
            "cost_sum_barrier_jac", std::vector<casadi::SX>{z, rho_b, obstacles_vector, num_obstacles},
...
void cilqr::get_init_traj() {
    ...
    for (int i = 0; i < N_; ++i) {
        ...
        auto next_res = f_(std::vector<casadi::DM>{casadi::DM(std::vector<double>(z_row.data(), z_row.data() + z_row.size()))});
...
double cilqr::get_total_cost(const Eigen::MatrixXd& x, const Eigen::MatrixXd& u) {
    double cost = 0.0;
    casadi::DM obstacles_dm(std::vector<double>(obstacles_vector_.data(), obstacles_vector_.data() + obstacles_vector_.size()));
    casadi::DM num_obs_dm(num_obstacles_);
    for (int k = 0; k < N_; ++k) {
        Eigen::RowVectorXd z_row(nx_ + nu_);
        z_row << x.row(k), u.row(k);
        casadi::DM z_row_dm(std::vector<double>(z_row.data(), z_row.data() + z_row.size()));
        casadi::DM Q_dm(std::vector<double>(Q_.data(), Q_.data() + Q_.size()));
        casadi::DM R_dm(std::vector<double>(R_.data(), R_.data() + R_.size()));
        casadi::DM ref_dm(std::vector<double>(ref_.row(k).data(), ref_.row(k).data() + ref_.cols()));
        ...
        if (num_c_ > 0) {
            if (!params_->use_Barrier) {
                ...
                auto cost_sum_res =
                    cost_sum_alm_(std::vector<casadi::DM>{z_row_dm, mu_dm, rho_dm, obstacles_dm, num_obs_dm});
                ...
                auto cost_sum_res =
                    cost_sum_barrier_(std::vector<casadi::DM>{z_row_dm, rho_dm, obstacles_dm, num_obs_dm});
                if (!cost_sum_res.empty()) {
                    casadi::DM cost_sum_dm = casadi::DM::densify(cost_sum_res[0]);
                    cost += static_cast<double>(cost_sum_dm(0));
                }
    ...
    casadi::DM Qf_dm(std::vector<double>(Qf_.data(), Qf_.data() + Qf_.size()));
    casadi::DM terminal_ref_dm(std::vector<double>(terminal_ref_.data(), terminal_ref_.data() + terminal_ref_.size()));
    auto term_res = terminal_l_(std::vector<casadi::DM>{casadi::DM(std::vector<double>(x_N.data(), x_N.data() + x_N.size())), Qf_dm, terminal_ref_dm});
...
void cilqr::get_cost_derivatives_and_Hessians() {
    casadi::DM Q_dm(std::vector<double>(Q_.data(), Q_.data() + Q_.size()));
    casadi::DM R_dm(std::vector<double>(R_.data(), R_.data() + R_.size()));
    casadi::DM ref_dm(std::vector<double>(ref_.data(), ref_.data() + ref_.size()));
    casadi::DM obstacles_dm(std::vector<double>(obstacles_vector_.data(), obstacles_vector_.data() + obstacles_vector_.size()));
    casadi::DM num_obs_dm(num_obstacles_);
    ...
    for (int k = 0; k < N_; ++k) {
        ...
        auto l_grad_res = l_jacobian_(std::vector<casadi::DM>{z_row_dm, Q_dm, R_dm, ref_dm});
        ...
        auto l_hess_res = l_hessian_(std::vector<casadi::DM>{z_row_dm, Q_dm, R_dm, ref_dm});
        ...
        if (num_c_ > 0) {
            ...
            if (params_->use_Barrier) {
                casadi::DM rho_dm(barrier_rho_);
                auto grad_res = cost_sum_barrier_jac_(std::vector<casadi::DM>{z_row_dm, rho_dm, obstacles_dm, num_obs_dm});
                auto hess_res = cost_sum_barrier_hess_(std::vector<casadi::DM>{z_row_dm, rho_dm, obstacles_dm, num_obs_dm});
                ...
                casadi::DM mu_dm(std::vector<double>(alm_mu_.row(k).data(), alm_mu_.row(k).data() + num_c_));
                casadi::DM rho_dm(alm_rho_);
                auto grad_res = cost_sum_alm_jac_(std::vector<casadi::DM>{z_row_dm, mu_dm, rho_dm, obstacles_dm, num_obs_dm});
                auto hess_res = cost_sum_alm_hess_(std::vector<casadi::DM>{z_row_dm, mu_dm, rho_dm, obstacles_dm, num_obs_dm});
                ...
                if (!c_.is_null()) {
                    auto c_val_res = c_(std::vector<casadi::DM>{z_row_dm, obstacles_dm, num_obs_dm});
                    if (!c_val_res.empty()) {
    ...
    casadi::DM Qf_dm(std::vector<double>(Qf_.data(), Qf_.data() + Qf_.size()));
    casadi::DM terminal_ref_dm(std::vector<double>(terminal_ref_.data(), terminal_ref_.data() + terminal_ref_.size()));
    auto term_l_x_res = terminal_l_x_(std::vector<casadi::DM>{casadi::DM(std::vector<double>(x_N.data(), x_N.data() + x_N.size())), Qf_dm, terminal_ref_dm});
    casadi::DM term_l_x_dm = casadi::DM::densify(term_l_x_res[0]);
    Eigen::Map<Eigen::VectorXd> term_l_x_vec(term_l_x_dm.ptr(), nx_);
    V_x_ = term_l_x_vec;

    auto term_l_xx_res = terminal_l_xx_(std::vector<casadi::DM>{casadi::DM(std::vector<double>(x_N.data(), x_N.data() + x_N.size())), Qf_dm, terminal_ref_dm});
...
bool cilqr::linear_search(double alpha) {
    ...
    for (int i = 0; i < N_; ++i) {
        ...
        auto next_x_res = f_(std::vector<casadi::DM>{casadi::DM(std::vector<double>(z_row.data(), z_row.data() + z_row.size()))});
